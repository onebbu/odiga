...
function CustomizedAccordions({duration, loginInfo, bottom}) {
    ...
    const handleChange = (panel) => (event, newExpanded) => { // 아코디언 열고 닫는 함수
    ...
    //       // 중복된 (courseDay, travelNum)을 가진 기존 데이터를 제거
    //       const filteredPrevData = prevData.filter(prevItem =>
    //           !updatedData.some(newItem => newItem.courseDay === prevItem.courseDay && newItem.travelNum === prevItem.travelNum )
    //       );

    //       // 기존 데이터에서 중복된 데이터를 제거하고 updatedData를 추가
    //       return [...filteredPrevData, ...updatedData];
    //     });
    //   }
    // }
    ...
    const onSave = (updatedData) => {
      ...
      return (
              ...
              if (duration && schedule.duration.includes(duration)) {
                return (
                  <Accordion key={schedule.id} expanded={expanded === `panel${schedule.id}`} onChange={handleChange(`panel${schedule.id}`)}>
                    ...
                    <AccordionDetails>
                        <DropContainer nickname={loginInfo.nickname} scheduleID={schedule.id} onSave={(data)=> onSave(data)} />
                    </AccordionDetails>
                  </Accordion>
                )
              } else {   return null;   }
        ...
        <BtnSave onClick={handleSaveClick} bot={bottom}>코스 저장하기</BtnSave> 
        </Position>
      );
};

//onSaveData={(data) => handleSaveData(data, schedule.id)}
